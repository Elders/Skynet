...
namespace Elders.Skynet.Transport.Tcp.Tests
{
    ...
    public class When_building_frame_from_exact_bytearray
    {
        Establish ctx = () =>
        {
            ...
            newFrame = new Frame();
            frameBytesStream = new MemoryStream(frameBytes);
        };

        Because of = () => { newFrame.Read(frameBytesStream); };

        It should_have_read_all_the_bytes = () => frameBytesStream.Position.ShouldEqual(frameBytes.Length);
        ...
        static Stream frameBytesStream;
